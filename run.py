...
#mnist_softmax = MNIST_softmax_data.DataSets(ambiguity=True)
...
for step in range(100001):
    ...
    for i in range(batchsize):
        ...
        #print('distance_to_anchor shape:', distance_to_anchor.shape)
        ...
        #pos_indices_unstack = tf.unstack(pos_indices)
        #neg_indices_unstack = tf.unstack(neg_indices)
        #print(pos_indices)

        for j in pos_indices:
            all_neg = np.where(distance_to_anchor[neg_indices] - distance_to_anchor[j] < alpha)[0]
            num_of_all_neg = all_neg.shape[0]
            #print(num_of_all_neg, neg_indices.shape[0])
            #assert num_of_all_neg < neg_indices.shape[0]
            if num_of_all_neg > 0:
...
#visualize.visualize(embed, mnist_softmax.train_x, mnist_softmax.train_y)
